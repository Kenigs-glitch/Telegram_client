...
`proxies.json` кладётся в **корень проекта** — рядом с `run.sh`:

```
telegram-multiacc/
├── run.sh
├── proxies.json            # список прокси (создать вручную)
└── proxy_assignments.json  # привязка аккаунт → прокси (создаётся автоматически)
```

Оба файла ищутся именно здесь (пути заданы в `run.sh`). Если `proxies.json` нет — прокси не предлагаются, идёт прямое подключение.
